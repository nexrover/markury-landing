...
export const notifyError = (error: any, request: Request, metadata?: Record<string, any>) => {
  if (process.env.NODE_ENV !== 'production') {
    return; // Opt out of executing and logging locally to avoid noise
  }

  if (Bugsnag.isStarted()) {
    Bugsnag.notify(error instanceof Error ? error : new Error(error?.message || error || 'Unknown Error'), function(event) {
